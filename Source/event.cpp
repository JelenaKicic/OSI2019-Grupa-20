...
void addEvent() //dodavanje dogadjaja
{
    ...
    } while (!date->setDate(stringToInt(day),stringToInt( month),stringToInt( year)));
    do
    {
    ...
    } while (!(time->setHours(stringToInt(hours)) && time->setMinutes(stringToInt(minutes))));
